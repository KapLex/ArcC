...
{
	// load texture
	Texture *t = malloc(sizeof(ARC_Texture));
	strcpy(t->fileName, texFileName);
...
int loadTexture(ARC_Texture *t, char* texFileName)
{
	...
	{
		log4c_category_log(textureLog, "Texture does not exist!");
		return EXIT_FAILURE;
	}
	...
	if (t->image == NULL)
	{
		log4c_category_log(textureLog, IMG_GetError());
		return EXIT_FAILURE;
	}
	...
	log4c_category_log("Texture has been loaded");
